...
_d = pp.Literal("d")
_ode_left = _d + pp.Or(variables) + pp.Literal("/") + _d + pp.Literal("t")
_operand = pp.Or([param_name, param_value])
_rhs = _operand + pp.OneOrMore(pp.Optional(pp.Or(_operators) + _operand))
_normal_eq_left = pp.Or(variables)
